...
export async function GET(request: NextRequest) {
  try {
    ...
    const expenses = await ExpenseModel.findWithFilters(filters);
    const total = await ExpenseModel.countWithFilters({
...
export async function POST(request: NextRequest) {
  try {
    ...
    const expense = await ExpenseModel.findById(expenseId.toString());
